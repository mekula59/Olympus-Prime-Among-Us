...
export interface AmongUsModuleDataView {
  ...
  commandZones: ZoneFeature[];
  bridgeReadouts: Readout[];
  railRumors: Readout[];
  commandWhispers: Readout[];
  crewRankings: CrewRanking[];
  crewRibbons: CrewRibbon[];
  crewProfiles: CrewProfile[];
  missionLogs: MissionLog[];
  reportMetrics: ReportMetric[];
  zoneDebriefs: ZoneDebrief[];
  reportReadouts: Readout[];
  legendEntries: LegendEntry[];
  incidentNotes: IncidentNote[];
  transmissions: Transmission[];
}

export function getAmongUsModuleData(): AmongUsModuleDataView {
  ...
  const commandZones: ZoneFeature[] = [
    {
      name: 'Podium Gallery',
      mood: 'Warm rivalry',
  ...
  const commandWhispers: Readout[] = getQuotesByContext('whisper').map((quote) => ({
    title: quote.locationLabel ?? quote.speakerLabel,
    detail: quote.text,
  }));

  const crewRankings: CrewRanking[] = rankingSnapshots
    .map((snapshot) => {
  ...
  const crewRibbons: CrewRibbon[] = (latestPublishedSession
  ...
  const crewProfiles: CrewProfile[] = rankingSnapshots.flatMap((snapshot) => {
    const player = getPlayerById(snapshot.playerId);
    if (!player) {
      return [];
    }
  ...
  const missionLogs: MissionLog[] = latestPublishedMatches.map((match, index) => ({
  ...
  const legendEntries: LegendEntry[] = getLegendMatches().map((match) => {
  ...
  const incidentNotes: IncidentNote[] = latestPublishedIncidents.map((incident) => ({
  ...
  const transmissions: Transmission[] = getQuotesByContext('transmission').map((quote) => ({
    title:
...
export function useAmongUsModuleData() {
  const { revision } = useRuntimeProductData();

  return useMemo(() => getAmongUsModuleData(), [revision]);
}

const initialAmongUsModuleData = getAmongUsModuleData();

export const commandStats = initialAmongUsModuleData.commandStats;
export const commandZones = initialAmongUsModuleData.commandZones;
export const bridgeReadouts = initialAmongUsModuleData.bridgeReadouts;
export const railRumors = initialAmongUsModuleData.railRumors;
export const commandWhispers = initialAmongUsModuleData.commandWhispers;
export const crewRankings = initialAmongUsModuleData.crewRankings;
export const crewRibbons = initialAmongUsModuleData.crewRibbons;
export const crewProfiles = initialAmongUsModuleData.crewProfiles;
export const missionLogs = initialAmongUsModuleData.missionLogs;
export const reportMetrics = initialAmongUsModuleData.reportMetrics;
export const zoneDebriefs = initialAmongUsModuleData.zoneDebriefs;
export const reportReadouts = initialAmongUsModuleData.reportReadouts;
export const legendEntries = initialAmongUsModuleData.legendEntries;
export const incidentNotes = initialAmongUsModuleData.incidentNotes;
export const transmissions = initialAmongUsModuleData.transmissions;
